var playState = {
	create: function () {
		...
		game.global.damageAble = true;
		game.global.damageTime = 0;
		game.global.facing = "left";
		game.global.control = "dad";
		game.global.pizza_count = 10;
		game.global.health_total = 2;
		game.global.health_current = 2;
		...
		for (i = 0; i < 60; i++) {
			var sidewalk = bottomSidewalks.create(
				i * 32 - 8,
				game.world.height - 18,
				"bottomSidewalk"
			);
		...
		for (i = 0; i < 60; i++) {
			var sidewalk = topSidewalks.create(
				i * 32 - 16,
		...
		for (i = 0; i < 4; i++) {
			var homeLedge = homeLedges.create(
				210,
				game.world.height - (204 + i * 120),
				"homeLedge"
			);
		...
		for (i = 0; i < 1; i++) {
			var trashCan = trashCans.create(
				400,
				game.world.height - (40 + 30),
				"trashCan"
			);
		...
		for (i = 0; i < game.global.pizza_count; i++) {
			...
			var drop_height =
				i == 2 ? game.world.height / 2 : game.world.height * 0.9;
		...
		/*** PLAYER DATA ***/
		/*** SCORE ***/
		game.global.scoreText = game.add.text(
			16,
			16,
			"Pizza: 0/" + game.global.pizza_count,
			{ fontSize: "32px", fill: "#000" }
		);
		game.global.scoreText.fixedToCamera = true;
		game.global.scoreText.cameraOffset.setTo(16, 16);
		/*** HEALTH ***/
		game.global.healthText = game.add.text(
			650,
			16,
			"Health: " +
				game.global.health_current +
				"/" +
				game.global.health_total,
			{ fontSize: "32px", fill: "#000" }
		);
		game.global.healthText.fixedToCamera = true;
		game.global.healthText.cameraOffset.setTo(650, 16);
	},
	...
	update: function () {
		...
		if (spacebarKey.isUp) {
			game.global.controlSwitchable = true;
		...
		function controls(leader, follower) {
			...
			var hitsideWalk = game.physics.arcade.collide(
				leader,
				bottomSidewalks
			);
			...
			if (
				cursors.left.isDown &&
				distanceFromLeader > leader.body.width + 10
			) {
				follower.animations.play("left");
				switch (leader) {
					case dad:
						follower.body.velocity.x = -220;
						break;
					case cat:
				...
				distanceFromLeader < -(follower.body.width + 10)
			) {
			...
			if (cursors.up.isUp) {
				game.global.jumped = false;
			} // allow one jump per key press
			if (
				cursors.up.isDown &&
				leader.body.touching.down &&
				(hitsideWalk || hittrashCan || hithomeLedge) &&
				game.global.jumped == false
			) {
			...
			function collectPizza(leader, pizza) {
				...
				game.global.score++;
				game.global.scoreText.text =
					"Pizza: " +
					game.global.score +
					"/" +
					game.global.pizza_count;
			}
			...
			if (dog.body.x == 0 && dog.body.touching.down) {
				...
				dog.body.x == game.world.width - 32 &&
				dog.body.touching.down
			) {
				dog.body.velocity.x = -100;
			...
			if (game.time.now - game.global.damageTime > 3000) {
				game.global.damageAble = true;
			}

			game.physics.arcade.overlap(leader, dog, damage, null, this);

			function damage() {
				if (game.global.damageAble) {
					game.global.damageAble = false;
                    game.global.damageTime = game.time.now;
					game.global.health_current--;
					game.global.healthText.text =
						"Health: " +
						game.global.health_current +
						"/" +
						game.global.health_total;
				}
				if (game.global.health_current <= 0) {
					gameOver();
				}
			}

			function invincibilityTimer() {
				damageAble;
			}
